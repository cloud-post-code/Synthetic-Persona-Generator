...
import { Search, Filter, MessageSquare, FileText, Star, Loader2, BookOpen, Library } from 'lucide-react';
import { personaApi } from '../services/personaApi.js';
import { Persona, PersonaFile } from '../models/types.js';

type TabType = 'discover' | 'mylibrary';
...
const PersonaLibraryPage: React.FC = () => {
  ...
  const loadStarred = async () => {
    try {
      ...
      setStarredPersonas(data.map(p => ({ ...p, avatarUrl: p.avatarUrl || p.avatar_url, createdAt: p.created_at || p.createdAt })));
    } catch (err) {
  ...
  useEffect(() => {
    setLoading(true);
    Promise.all([loadLibrary(), loadStarred()]).finally(() => setLoading(false));
  }, []);
  ...
  const handleToggleStar = async (persona: Persona) => {
    const id = persona.id;
    const starred = (persona as Persona & { starred?: boolean }).starred ?? starredPersonas.some(p => p.id === id);
    setTogglingStarId(id);
    try {
      if (starred) {
        await personaApi.unstar(id);
        setStarredPersonas(prev => prev.filter(p => p.id !== id));
        setLibraryPersonas(prev => prev.map(p => p.id === id ? { ...p, starred: false } : p));
      } else {
        await personaApi.star(id);
        setStarredPersonas(prev => [...prev, { ...persona, starred: true }]);
  ...
  const list = activeTab === 'discover' ? libraryPersonas : starredPersonas;
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10 w-full relative">
      <div className="mb-8">
        ...
        <p className="text-gray-500">Discover public personas and save them to your library.</p>
      </div>
      ...
      ) : activeTab === 'mylibrary' && starredPersonas.length === 0 ? (
        <div className="text-center py-20 bg-gray-50 rounded-3xl border border-dashed border-gray-200">
          <Star className="w-12 h-12 text-gray-300 mx-auto mb-4" />
          <h3 className="text-lg font-medium text-gray-900">No starred personas</h3>
          <p className="text-gray-500">Star personas from Discover to add them here.</p>
          <button onClick={() => setActiveTab('discover')} className="mt-4 text-indigo-600 font-semibold hover:underline">
            Browse Discover
          </button>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {filtered.map((persona) => (
            <LibraryPersonaCard
              key={persona.id}
              persona={persona}
              showUnstar={activeTab === 'mylibrary'}
...
const LibraryPersonaCard: React.FC<{
  ...
  onToggleStar: () => void;
  onViewFiles: () => void;
}> = ({ persona, showUnstar, isTogglingStar, onToggleStar, onViewFiles }) => {
  const navigate = useNavigate();
  const avatarUrl = persona.avatarUrl || persona.avatar_url;
  const createdAt = persona.createdAt || persona.created_at;
  const starred = (persona as Persona & { starred?: boolean }).starred ?? showUnstar;
        ...
        <div className="absolute top-4 right-4 flex gap-2 z-10">
          <button
            ...
            className={`p-2 rounded-lg transition-colors shadow-sm cursor-pointer ${
              starred ? 'bg-amber-100 text-amber-600 hover:bg-amber-200' : 'bg-white/80 backdrop-blur text-gray-400 hover:bg-white hover:text-amber-600'
            }`}
            title={starred ? 'Unstar' : 'Star'}
          >
            {isTogglingStar ? <Loader2 className="w-4 h-4 animate-spin" /> : <Star className={`w-4 h-4 ${starred ? 'fill-current' : ''}`} />}
          </button>
          <button
