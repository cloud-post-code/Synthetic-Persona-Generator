import type { UiNode } from './intents.js';

/** Canonical UI map — keep in sync with backend/src/voice/uiMapData.ts */
...
function queryMatches(
  nodeQuery: Record<string, string> | undefined,
  searchParams: URLSearchParams
): boolean {
  if (!nodeQuery || Object.keys(nodeQuery).length === 0) return true;
  for (const [k, v] of Object.entries(nodeQuery)) {
    if ((searchParams.get(k) || '') !== v) return false;
  }
  return true;
}

/** Resolve pathname + search to a UI node id */
...
export function neighborsOf(nodeId: string): UiNode[] {
  const node = getNodeById(nodeId);
  if (!node) return [];
  return node.transitions.map((t) => getNodeById(t.to)).filter((n): n is UiNode => !!n);
}

export type AuthSnapshot = { isAuthenticated: boolean; isAdmin: boolean };

function nodeAllowedForAuth(node: UiNode, auth: AuthSnapshot): boolean {
  const p = node.prerequisites?.auth;
  if (!p) return true;
  if (p === 'user') return auth.isAuthenticated;
  if (p === 'admin') return auth.isAuthenticated && auth.isAdmin;
  return true;
}

/** Token-efficient prompt block for Gemini */
export function buildUiMapForPrompt(currentNodeId: string | null, auth: AuthSnapshot): string {
  const lines: string[] = ['### UI_MAP'];

  if (currentNodeId) {
    const cur = getNodeById(currentNodeId);
    if (cur && nodeAllowedForAuth(cur, auth)) {
      lines.push('CURRENT_NODE:');
      lines.push(`  id: ${cur.id}`);
      lines.push(`  title: ${cur.title}`);
      lines.push(`  path: ${cur.path}`);
      if (cur.query) lines.push(`  query: ${JSON.stringify(cur.query)}`);
      lines.push(`  purpose: ${cur.purpose}`);
      lines.push(`  whenToUse: ${cur.whenToUse.join('; ')}`);
      lines.push('  transitions:');
      for (const tr of cur.transitions) {
        lines.push(`    - to:${tr.to} via:${tr.via} label:${tr.label}${tr.targetId ? ` targetId:${tr.targetId}` : ''}`);
      }
      lines.push('  goals:');
      for (const g of cur.goals) {
        lines.push(`    - id:${g.id} desc:${g.description} completion:${JSON.stringify(g.completion)}`);
      }
      lines.push('');
      lines.push('NEIGHBORS (1-hop):');
      for (const n of neighborsOf(currentNodeId)) {
        if (!nodeAllowedForAuth(n, auth)) continue;
        const q = n.query ? `?${new URLSearchParams(n.query).toString()}` : '';
        lines.push(`  - ${n.id} | ${n.title} | ${n.path}${q} | ${n.purpose}`);
      }
    }
  } else {
    lines.push('CURRENT_NODE: unknown');
  }

  lines.push('');
  lines.push('INDEX (all nodes):');
  for (const n of UI_NODES) {
    if (!nodeAllowedForAuth(n, auth)) continue;
    const q = n.query ? `?${new URLSearchParams(n.query).toString()}` : '';
    const pre = n.prerequisites?.auth || 'none';
    lines.push(`  - ${n.id} | ${n.path}${q} | ${n.title} | ${n.purpose} | auth:${pre}`);
  }
